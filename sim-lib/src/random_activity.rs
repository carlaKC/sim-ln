...
/// Seeded random number generator `T`, i.e. `Option<Box<dyn RngCore + Send>>` wrapped in `Arc<Mutex<T>>`
/// for thread-safe shared access with interior mutability
type MutRng = Arc<Mutex<Box<dyn RngCore + Send>>>;
...
/// and track node capacity within the network. The `NetworkGraphView` also keeps a handle on an optional seed that
/// allows it to deterministically pick nodes. Tracking nodes in the network is memory-expensive, so we
...
mod tests {
    mod test_network_graph_view {
        ...
        fn test_new() {
            ...
            for i in 0..2 {
                assert!(matches!(
                    NetworkGraphView::new(create_nodes(i, 42 * (i as u64 + 1)), new_mut_rng(None)),
            ...
            assert!(matches!(
                NetworkGraphView::new(nodes, new_mut_rng(None)),
                Err(RandomActivityError::InsufficientCapacity { .. })
            ));

            // All of them are 0
            assert!(matches!(
                NetworkGraphView::new(create_nodes(2, 0), new_mut_rng(None)),
                Err(RandomActivityError::InsufficientCapacity { .. })
            ));

            // Otherwise we should be good
            assert!(NetworkGraphView::new(create_nodes(2, 42), new_mut_rng(None)).is_ok());
        }
        ...
        fn test_sample_node_by_capacity() {
            ...
            let view = NetworkGraphView::new(nodes, new_mut_rng(None)).unwrap();
    ...
    mod payment_activity_generator {
        ...
        fn test_payment_amount() {
            ...
            let pag = RandomPaymentActivity::new(
                source_capacity,
                expected_payment,
                1.0,
                new_mut_rng(Some(u64::MAX)),
