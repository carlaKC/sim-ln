...
pub trait Interceptor: Send + Sync {
    ...
    async fn notify_resolution(&self, _res: InterceptResolution) -> Result<(), Option<String>> {
        Ok(())
    }
...
pub struct InterceptRequest {
    ...
    pub response:
        tokio::sync::mpsc::Sender<Result<Result<CustomRecords, ForwardingError>, Option<String>>>,
}
...
impl InterceptRequest {
    fn new(
        ...
        response: tokio::sync::mpsc::Sender<
            Result<Result<CustomRecords, ForwardingError>, Option<String>>,
        >,
    ) -> Self {
        Self {
            ...
            'get_resp: for i in 0..interceptors.len() {
                ...
                resp = receiver.recv() =>{
                        match resp {
                            ...
                            Some(Err(e)) => {
                                return Err((fail_idx, ForwardingError::InterceptorError(format!("{:?}",e))))
                            },
                            None => break 'get_resp
                        }
                    },
    ...
    for (i, hop) in route.hops[0..=resolution_idx].iter().enumerate().rev() {
        ...
        for interceptor in interceptors.iter() {
            ...
            interceptor
                ...
                .await
                .map_err(|e| ForwardingError::InterceptorError(e.unwrap_or_default()))?;
        }
