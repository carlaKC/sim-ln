    ...
    fn dispatch_consumers(
            ...
            .filter(|(id, _)| consuming_nodes.contains(id))
        {
            ...
            tasks.spawn(async move {
                ...
                if let Err(e) = consume_events(ce_node, clock, receiver, ce_output_sender).await {
                    ce_shutdown.trigger();
...
mod tests {
    ...
    async fn test_shutdown_timeout() {
        ...
        let result = simulation.run(&[activity_definition]).await;
    ...
    async fn test_shutdown_on_error() {
        ...
        let start = std::time::Instant::now();
        let _ = simulation.run(&[activity_definition]).await;
        let elapsed = start.elapsed();
    ...
    async fn test_shutdown_multiple_activities() {
        ...
        let start = std::time::Instant::now();
        let result = simulation.run(&[activity_1, activity_2]).await;
        let elapsed = start.elapsed();
    ...
    async fn test_shutdown_multiple_activities_with_error() {
        ...
        // Run the simulation (should be interrupted by the error)
        let start = std::time::Instant::now();
        let _ = simulation.run(&[activity_1, activity_2]).await;
        let elapsed = start.elapsed();
